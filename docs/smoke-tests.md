...
- Action: Run `install-agent-surface --global --skill commit-plan` from
...
- Action: After `chezmoi apply`, run `install-agent-surface --target <temp-normal-repo> --skill commit-plan` from fish, then run it again from bash with `--chezmoi --target <temp-chezmoi-repo> --skill openai-docs`.
  ...
  target receives `AGENTS.md`, `AGENTS.repo.md`, `plans/`, and
  `dot_agents/skills/openai-docs/SKILL.md` without creating `.agents/`,
  both shells resolve the command from `~/scripts`, and `--chezmoi`
  installs `chezmoi` into `$HOME/.local/bin` first when the command is
  missing.
...
- Action: Open Vim or Neovim on a Python file that contains nested calls and colon-terminated block headers such as `async def`, `if`, `try`, or `async with`, then scroll the mouse wheel up and down.
  Expected: Scroll-down from a block header enters the first indented statement in that block, while scrolling from ordinary statements jumps to the next or previous nonblank line whose indentation is the same or lower.
...
- Action: Run `/home/kevin/scripts/apply-desktop-display-layout`, reload i3, and inspect `xrandr --query`.
  Expected: `DVI-D-0` is 2048x1152 at `+0+0`, `DP-2` is 3840x2160 at `+2048+0`, `HDMI-0` is 1920x1080 at `+5888+0`, and i3 shows bars on all three monitors with the new middle 4K bar using the combined active `spotify`, `volume`, and `calendar` blocks.
- Action: Start Spotify, play a track, run /home/kevin/.config/i3blocks/scripts/spotify_mpris from an X session, then left-, middle-, and right-click the middle 4K Spotify block.
  Expected: The script prints three i3blocks lines with metadata full text like `Artist - Title`, short text as the title, and pywal `color7`; the middle 4K bar renders that as ` Artist - Title` using the block label, and clicks route to PlayPause, Previous, and Next on `org.mpris.MediaPlayer2.spotify`.
