# ExecPlan: Rebrand `push-dirty` Skill To `Commit Plan`
...
- [x] Update the skill metadata, heading, and trigger text to use `push-plan` / `Commit Plan`.
...
- The human-facing skill title should become `Commit Plan`.
- The workflow remains read-only commit planning; no behavioral expansion is required.

## Review Notes

- Renamed the live commit-planning skill from `push-dirty` to `push-plan` by moving `SKILL.md` under `dot_agents/skills/push-plan/` and updating the front matter, title, and trigger text to say `Commit Plan`.
...
- Verification completed with:
  ...
  - `rg -n "push-plan|Commit Plan" dot_agents plans`
