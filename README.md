...
| `install-agent-surface --global --all-skills` after `chezmoi apply`, or `./scripts/executable_install-agent-surface --global --all-skills` from this clone | Install the global AI surface for Codex: `~/.codex/AGENTS.md`, `~/.agents/AGENTS.md`, `~/.agents/skills/`, and `~/.agents/system-design/`. Fish gets completions for the installed command, and bash can run the same command through `~/scripts` as a fallback shell. | `--global` chooses the home-level install. `--target <repo>` scaffolds a repo-local `AGENTS.md`, `AGENTS.repo.md`, `plans/`, and local skills folder. `--chezmoi` writes `dot_agents/skills/` instead of `.agents/skills/` and auto-installs `chezmoi` into `$HOME/.local/bin` when missing. `--skill NAME` copies selected tracked skills, `--all-skills` copies every tracked skill with a `SKILL.md`, and `--force` overwrites existing AGENTS files or copied skill directories. |
